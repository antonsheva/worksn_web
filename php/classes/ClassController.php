...
class ClassController{
    function __construct(){
        global $G, $M, $U, $AU, $A, $P, $AC_img, $LOG, $U_review;
        $act = $P->AGet(STR_ACT);
        if($act){
            $G->act = $act;
            if ($G->user_id)$U->checkSessionToken($G->act);
            switch($act){
                case ACT_SEND_EXCEPTION           : $LOG->saveCrashDataToFile();        break;
                case ACT_SEND_LOG_ACTIVITY        : $LOG->saveLogActivityDataToFile();  break;
                case ACT_SEND_LOG_SERVICE         : $LOG->saveLogServiceDataToFile();   break;
                case ACT_GET_ENVIRONMENT_DATA     : $this->getEnvData();                break;
                case ACT_REFRESH_SESSION          : $this->refreshSession();            break;
                case ACT_GET_SETTING_DATA         : $this->getSettingPageData();        break;
